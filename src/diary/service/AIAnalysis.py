...
def analyze_diary_entry(content: str) -> str:
    stream = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "user",
                "content": f'당신은 심리학자입니다. 사용자가 작성한 일기를 분석하여 주요 감정을 식별하고(일기를 분석하여 주요 감정을 식별한 부분에는 키워드가 필요 없습니다.), 이를 바탕으로 공감적이고 실용적인 조언을 제공해주세요(공감적이고 실용적 조언마다 간단한 핵심 키워드 : 실용적인 조언 의 형태로 반환해 주세요. 예시: 휴식을 우선시하세요: 휴식은 생산성을 높이는 중요한 요소입니다. 잠깐이라도 눈을 감고 휴식을 취해보세요.). 그리고 일기를 분석하고 감정을 식별한 내용과 공감적이고 실용적인 내용 사이에 "-" 3개로 구분해 주세요. 구분은 일기를 분석하고 감정을 식별한 내용과 공감적이고 실용적인 내용 사이에 공백없이 한 번만 입력해주세요.(예시: 분석하고 감정을 식별한 내용.---공감적이고 실용적인 내용)\n 일기 내용 : {content}',
            }
        ],
        stream=True,
    )
